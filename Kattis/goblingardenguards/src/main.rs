...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    let g = scan.token::<usize>().unwrap();
    let mut goblins = vec![vec![0;10_001];10_001];
    // let mut pos = HashMap::new();
    for _ in 0..g {
        let x = scan.token::<usize>().unwrap();
        let y = scan.token::<usize>().unwrap();
        goblins[y][x] += 1;
        // match pos.get_mut(&(y, x)) {
        //     Some(count) => {*count += 1; },
        //     None => { pos.insert((y, x), 1); }
        // }
    }
    ...
    for _ in 0..m {
        ...
        let lx = (x - r).max(0) as usize;
        let rx = (x + r ).min(10_000) as usize;
        let ly = (y - r).max(0) as usize;
        let ry = (y + r).min(10_000) as usize;
        for i in ly..=ry {
            for j in lx..=rx {
                if ((i as isize) - y).pow(2) + ((j as isize) - x).pow(2) <= r.pow(2) {
                    goblins[i][j] = 0;
                }
            }
        }
    }
    let res: usize = goblins.into_iter().map(|r| r.into_iter().sum::<usize>()).sum();
    writeln!(out, "{}", res);
}
