...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    loop {
        let (sweet, sour) = (scan.token::<u32>().unwrap(), scan.token::<u32>().unwrap());
        if sweet == 0 && sour == 0 {
            return;
        }
        let mut res = match sweet.cmp(&sour) {
            std::cmp::Ordering::Less =>"Left beehind.",
            std::cmp::Ordering::Greater => "To the convention.",
            std::cmp::Ordering::Equal => "Undecided.",
        };
        if sweet + sour == 13 {
