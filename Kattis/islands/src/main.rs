...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    for _ in 0..t {
        ...
        let mut v = Vec::new();
        for _ in 0..12 {
            v.push(scan.token::<usize>().unwrap());
        }
        let mut mins = vec![vec![std::usize::MAX;12];12];
        for i in 0..12 {
            for j in i..12 {
                if j > i {
                    mins[i][j] = v[j].min(mins[i][j-1]);
                } else {
                    mins[i][j] = v[j];
                }
            }
        ...
        for i in 1..11 {
            for j in i..11 {
                if mins[i][j] > v[i-1] && mins[i][j] > v[j+1] {
                    res += 1;
                }
            }
        }
        writeln!(out, "{} {}", k, res).unwrap();
    }
}
