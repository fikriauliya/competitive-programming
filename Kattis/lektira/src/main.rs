...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    let mut min = line.clone();
    for i in 1..line.len()-1 {
        for j in i+1..line.len() {
            let mut a = vec![' '; i];
            let mut b = vec![' '; j-i];
            let mut c = vec![' '; line.len()-j];
            a.copy_from_slice(&line[..i]);
            b.copy_from_slice(&line[i..j]);
            c.copy_from_slice(&line[j..]);
            a.reverse();
            b.reverse();
            c.reverse();
            let d = [a, b, c].concat();
    ...
    let res:String = min.iter().collect();
    writeln!(out, "{}", res);
}
