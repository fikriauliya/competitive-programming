...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    let prev = scan.token::<usize>().unwrap();
    let transaction = 97; //a
    let v = (prev * 31 + transaction).rem_euclid(modu);
    dbg!(&v);
    dbg!(v * 7);
    let token = (100000_000_000 - v * 7).rem_euclid(modu);
    writeln!(out, "a {}", token);

    let prev = (v * 7 + token).rem_euclid(modu);
    dbg!(&prev);
