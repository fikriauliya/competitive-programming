...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    for i in 0..n {
        ...
        for j in 0..n {
            m[i][j] = line[j];
            if line[j] == 'H' { houses.push((i,j)); }
            if line[j] == 'S' { spies.push((i,j)); }
        }
    }
    let mut res = 0;
    for spy in &spies {
        ...
        for house in &houses {
            let d = ((house.0 as isize) - (spy.0 as isize)).abs() +
                ((house.1 as isize) - (spy.1 as isize)).abs();
            dist = dist.min(d as usize);
        }
        res = res.max(dist);
    }
    writeln!(out, "{}", res);
