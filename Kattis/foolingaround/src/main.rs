...
fn sieve(n: usize) -> Vec<bool> {
    ...
    primes
    // primes.iter().enumerate().filter(|(p, v)| **v).map(|p| p.0).collect()
}
...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    // // let subs: HashSet<usize> = primes.iter().map(|p| p-1).collect();
    // // let subs: HashMap<usize, ()> = subs.into_iter().map(|k| (k, ())).collect();
    // let elapsed = now.elapsed();
    // dbg!(&elapsed, &primes.len());
