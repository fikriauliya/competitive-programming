...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    loop {
        ...
        for _ in 0..n {
            let src = scan.token::<usize>().unwrap();
            let dest = scan.token::<usize>().unwrap();
            let start = scan.token::<usize>().unwrap();
            let duration = scan.token::<usize>().unwrap();
            let end = start + duration;
            calls.push(Call{src, dest, start, end});
        }
        for _ in 0..m {
            let start = scan.token::<usize>().unwrap();
            let duration = scan.token::<usize>().unwrap();
            ...
            for i in 0..n {
                ...
                if start >= c.start && start < c.end {
                    res += 1;
                } else if start < c.start && end > c.start {
                    res += 1;
                }
            }
            writeln!(out, "{}", res);
