...
fn rec(i: usize, accum: usize, primes: &Vec<usize>, res: &mut HashSet<usize>, x: &usize, y:&usize) {
    if accum > *y { return ; }
    if accum >= *x && accum <= *y { res.insert(accum); }
    for i in i..primes.len() {
        rec(i, accum * primes[i], primes, res, x, y);
    }
}
...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    loop {
        ...
        for _ in 0..n {
            let prime = scan.token::<usize>().unwrap();
            primes.push(prime);
        }
        let x = scan.token::<usize>().unwrap();
        let y = scan.token::<usize>().unwrap();
        let mut res = HashSet::new();
        rec(0, 1, &primes, &mut res, &x, &y);
        if res.is_empty() {
            writeln!(out, "none");
        } else {
            let mut res: Vec<usize> = res.into_iter().collect();
            res.sort();
            let res: Vec<String> = res.into_iter().map(|r| r.to_string()).collect();
