...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut UnsafeScanner<R>, out: &mut W) {
    ...
    for tt in 0..t {
        ...
        let mut max = Vec::new();
        max.push(nums[0]-1);
        max.push(k - nums[nums.len()-1]);
        if diffs.len() >= 1 {
            max.push(diffs[diffs.len()-1]/2);
        }
        if diffs.len() >= 2 {
            max.push(diffs[diffs.len()-2]/2);
        }
        max.sort();
        // dbg!(&max);
        let res = max[max.len()-1] + max[max.len()-2];
        writeln!(out, "Case #{}: {}", tt+1, (res as f64)/(k as f64)).unwrap();
    }
}
